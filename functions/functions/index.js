...
/**
 * decode the toke into an object
 * @param {String} token 
 */
function getAuthData (token) {
	try {
		const secret = functions.config().auth.secret;
...
exports.sendMessage = functions.https.onCall((data) => {
	const {uid, msg, type, thread, authorization} = data;
	const auth = getAuthData(authorization);
